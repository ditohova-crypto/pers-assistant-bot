...
GIGACHAT_API_URL_ALT = "https://gigachat.ru/api/v1/chat/completions"
GIGACHAT_AUTH_URL = "https://ngw.devices.sberbank.ru:9443/api/v2/oauth"

SYSTEM_PROMPT = (
    "Ты — AI-ассистент по имени Гига. Ты — программа, созданная для помощи.\n"
    ...
    "Всегда отвечай от своего имени (Гига) и помни, что Диана — отдельный человек.\n"
...
async def get_gigachat_token() -> str:
    ...
    try:
        # GigaChat использует Authorization Key (Base64 от Client ID + Secret)
        ...
        async with aiohttp.ClientSession() as session:
            # Отключаем проверку SSL (GigaChat может иметь самоподписанный сертификат)
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"Auth success: {result.keys()}")
                    return result.get("access_token", "")
                else:
                    error_text = await response.text()
                    print(f"Auth error: {response.status} - {error_text[:200]}")
                    return ""
...
async def ask_ai(user_message: str, extra_context: str = "") -> str:
    ...
    if not token:
        return "Ошибка авторизации в GigaChat. Проверь настройки API."
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # Пробуем основной URL, если не работает — альтернативный
            urls_to_try = [GIGACHAT_API_URL, GIGACHAT_API_URL_ALT]
            response = None
            ...
            if response is None:
                return "Ошибка соединения с GigaChat. Попробуй позже."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет, Диана! 👋 Я Гига, твой ассистент от Сбера.\n\n"
        "Что я умею:\n"
...
async def main():
    ...
    async def health(request):
        return web.Response(text="OK - Гига для Дианы (GigaChat)")
    ...
    print("Гига запущена для Дианы! (GigaChat)")
