...
MORNING_TIME = time(6, 0)    # 9:00 МСК
AFTERNOON_TIME = time(12, 0)  # 15:00 МСК
EVENING_TIME = time(18, 0)    # 21:00 МСК
...
async def search_and_summarize(query: str) -> str:
    ...
    for result in results[:2]:
        ...
        if content:
            contents.append(f"Источник: {result['title']}
{content[:1500]}")

    if not contents:
        links_text = "\n".join([f"• {r['title']}: {r['url']}" for r in results])
        return f"Нашел информацию по запросу:\n{links_text}"
...
async def ask_kimi(user_message: str, system_prompt: str = None, web_context: str = None) -> str:
    """Запрос к Kimi API с опциональным контекстом из интернета."""
    headers = {
...
def needs_web_search(text: str) -> bool:
    ...
    factual_patterns = [
        r"какая\s+сейчас\s+",
        r"сколько\s+сейчас\s+",
        r"что\s+нового\s+",
        r"последние\s+новости\s+",
        r"курс\s+\w+",
        r"погода\s+в\s+",
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "💡 Просто задавай вопросы — если нужна свежая информация, "
        "я сам найду её в интернете!\n\n"
        "Примеры:\n"
        "• «Какие новости в IT сегодня?»\n"
        "• «Курс доллара сейчас»\n"
        "• «Погода в Москве»\n"
        "• «Что такое новый закон о НПД?»"
    )
...
async def add_task_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Задача #{task_id} добавлена: {task_text}")
...
async def list_tasks_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if completed_today:
        text += f"\n🟢 Сегодня выполнено: {len(completed_today)}\n"
...
async def clear_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🧹 Очищено {cleared} выполненных задач. Активных: {len(active)}")
...
async def search_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    system_prompt = (
        "Ты — личный ассистент. Ответь на вопрос пользователя, используя "
        "предоставленную информацию из интернета. Давай краткий, структурированный ответ. "
        "Если информация неполная, скажи об этом. Укажи источники, если они есть."
    )

    answer = await ask_kimi(f"Вопрос: {query}\n\nПроанализируй информацию и ответь.", system_prompt, web_context)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if need_search:
        ...
        if web_context:
            await update.message.reply_text("📡 Нашёл информацию, анализирую...")

    system_prompt = (
        "Ты — личный ассистент пользователя. Ты помогаешь организовывать дела, "
        "даёшь советы по продуктивности, поддерживаешь мотивацию. "
        "Ты знаешь о задачах пользователя и можешь на них ссылаться. "
        "Отвечай дружелюбно, по-русски, кратко и по делу. "
        "Если используешь информацию из интернета — укажи это."
    )

    active_tasks = get_active_tasks()
    if active_tasks:
        tasks_context = "Активные задачи пользователя:\n" + "\n".join([f"- {t['text']}" for t in active_tasks])
        system_prompt += f"\n\n{tasks_context}"
...
async def afternoon_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    if tasks:
        text = "☀️ Полдень! Проверь прогресс:\n\n"
        text += f"✅ Выполнено сегодня: {len(completed)}\n"
        text += f"⬜ Осталось: {len(tasks)}\n\n"
        for t in tasks:
            text += f"  • {t['text']}\n"
        text += "\nТы справишься! 🚀"
    else:
        text = "☀️ Полдень! Все задачи на сегодня выполнены — ты супергерой! 🦸‍♀️"
...
async def evening_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"📊 Сегодня выполнено: {len(completed)}\n"

    if tasks:
        text += f"📋 Перенесено на завтра: {len(tasks)}\n"
        for t in tasks:
            text += f"  • {t['text']}\n"
...
def main():
    ...
    print("🤖 Бот запущен с поиском в интернете!")
    print("📡 Автопоиск включён для актуальных вопросов")
