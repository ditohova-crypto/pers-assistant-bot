...
API_KEY = os.environ.get("API_KEY", "")  # OpenRouter ключ
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(API_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=40)) as response:
                if response.status == 200:
                    ...
                    return "Ошибка соединения с сервером AI. Попробуй позже."
...
async def main():
    ...
    async def health(request):
        return web.Response(text="OK - Кими для Дианы (OpenRouter)")
    ...
    print("Кими запущен для Дианы! (OpenRouter + Kimi K2.6)")
