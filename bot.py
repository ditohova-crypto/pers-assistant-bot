...
async def get_gigachat_token() -> str:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(GIGACHAT_AUTH_URL, headers=headers, data=data, timeout=aiohttp.ClientTimeout(total=20)) as response:
                if response.status == 200:
                    result = await response.json()
                    return result.get("access_token", "")
                else:
                    error_text = await response.text()
                    print(f"Auth error: {response.status} - {error_text[:100]}")
                    return ""
...
async def ask_ai(user_message: str, extra_context: str = "") -> str:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(GIGACHAT_API_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=40)) as response:
                if response.status == 200:
                    result = await response.json()
