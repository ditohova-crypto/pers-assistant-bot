...
# Пробуем оба URL Moonshot
KIMI_API_URLS = [
    "https://api.moonshot.ai/v1/chat/completions",  # Международный
    "https://api.moonshot.cn/v1/chat/completions",  # Китайский
]
MODEL = "kimi-k2-6"  # или "moonshot-v1-8k"
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(api_url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=40)) as response:
                    if response.status == 200:
                        ...
                        print(f"[OK] Moonshot работает через: {api_url}")
                        return answer
                    else:
                        error_text = await response.text()
                        last_error = f"{api_url}: {response.status} - {error_text}"
                        print(f"[FAIL] {last_error}")
        except Exception as e:
            last_error = f"{api_url}: {str(e)}"
            print(f"[ERROR] {last_error}")

    return "Ошибка API: " + last_error
...
async def main():
    ...
    async def health(request):
        return web.Response(text="OK - Кими для Дианы (Moonshot)")
