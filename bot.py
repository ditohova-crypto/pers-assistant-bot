...
# Личность бота (постоянный системный промпт)
BOT_PERSONALITY = (
...
async def ask_ai(user_message: str, system_prompt: str = None, web_context: str = None) -> str:
    ...
    # Постоянная личность бота
    messages.append({"role": "system", "content": BOT_PERSONALITY})

    # Дополнительный системный промпт (если есть)
    if system_prompt:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        ...
        return
    welcome_text = (
        "Привет! Я твой личный ассистент Kimi K2.6 с доступом к интернету.\n\n"
        "Вот что я умею:\n"
        ...
        "Просто задавай вопросы — я помню наш разговор и могу искать актуальную информацию!\n\n"
        "Примеры:\n"
...
async def main():
    ...
    try:
        ...
        async def health_handler(request):
            return web.Response(text="OK - Kimi K2.6 Bot with memory is running")
        ...
        print("Бот Kimi K2.6 с памятью запущен!")
